...
def regenerate_file(filepath):
    ...
    real output of the Zen C program. The hand-written C example is
    illustrative and may differ in details, so it is not required to agree."""
    ...
    with tempfile.TemporaryDirectory() as td:
        for lineno, zc_code in zc_blocks:
            ...
            if expected is not None and zc_out_norm != expected:
                errors.append(
                    f"{relpath}:{lineno}: output != expected (run --regenerate)\n"
                    f"  Expected: {expected!r}\n"
                    f"  Got:      {zc_out_norm!r}"
                )

            # The compiler C backend and the hand-written C example are
            # validated too, but their differences are warnings: the docs QA is
            # about the documented examples, not the C toolchain.
            try:
                transpile_c = os.path.join(td, "zc_t.c")
                ok, err = transpile_zc(zc_code, transpile_c)
                if not ok:
                    warnings.append(f"{relpath}:{lineno}: ZC transpile failed\n{err[:300]}")
                else:
                    ...
                    if not ok:
                        warnings.append(f"{relpath}:{lineno}: transpiled C compile failed\n{err[:300]}")
                    else:
                        ok, stdout_tc, stderr = run_binary(transpile_out)
                        if not ok:
                            warnings.append(f"{relpath}:{lineno}: transpiled C run failed: {stderr}")
                        elif zc_out_norm != normalize_output(stdout_tc):
                            warnings.append(
                                f"{relpath}:{lineno}: ZC vs transpiled C output differ\n"
                                f"  ZC:  {zc_out_norm!r}\n"
                                f"  TC:  {normalize_output(stdout_tc)!r}"
                            )
            except OSError:
                pass
            ...
            for clineno, c_code in c_blocks:
                ...
                if not ok:
                    warnings.append(f"{relpath}:{clineno}: C example compile failed\n{err[:300]}")
                    continue
                ok, stdout_c, stderr = run_binary(c_out)
                if not ok:
                    warnings.append(f"{relpath}:{clineno}: C example run failed: {stderr}")
                elif expected is not None and normalize_output(stdout_c) != expected:
                    warnings.append(
                        f"{relpath}:{clineno}: C example output differs from documented output\n"
                        f"  Expected: {expected!r}\n"
                        f"  Got:      {normalize_output(stdout_c)!r}"
                    )
...
def main():
    ...
    for filepath in files:
        if not os.path.isfile(filepath):
            continue
        total += 1
        # A file-level marker skips comparison docs whose examples are
        # illustrative or use APIs that need rewriting (marked in the file).
        with open(filepath, encoding="utf-8") as f:
            if "zc-check: skip-file" in f.read():
                skipped += 1
                continue
        if regenerate:
